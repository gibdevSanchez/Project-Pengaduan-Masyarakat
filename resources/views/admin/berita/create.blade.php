...
<div class="max-w-2xl space-y-5">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.berita.index') }}" class="text-stone-400 hover:text-stone-600 transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
        </a>
        <h1 class="text-xl font-bold text-stone-900">Buat Berita Baru</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.berita.store') }}"
        ...
        <div>
            ...
            <input type="text" name="judul" value="{{ old('judul') }}" required
                   class="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-200 @error('judul') border-red-400 @enderror">
        ...
        <div>
            ...
            <textarea name="isi" rows="6" required
                      class="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-200 @error('isi') border-red-400 @enderror">{{ old('isi') }}</textarea>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="kategori" required class="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm focus:outline-none">
            ...
            <div>
                ...
                <select name="format" required class="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm focus:outline-none">
                    <option value="biasa">Biasa</option>
                    <option value="besar">Besar (Headline)</option>
                </select>
            </div>
        </div>

        <div>
            <label class="block text-sm font-medium text-stone-700 mb-1">Terkait Pengaduan <span class="text-stone-400 font-normal">(opsional)</span></label>
            <select name="id_pengaduan" class="w-full rounded-lg border border-stone-200 px-3 py-2 text-sm focus:outline-none">
                <option value="">— Tidak ada —</option>
                @foreach($pengaduanList as $p)
                <option value="{{ $p->id_pengaduan }}" {{ old('id_pengaduan') == $p->id_pengaduan ? 'selected' : '' }}>
        ...
        <div class="rounded-xl border border-stone-200 overflow-hidden">
            ...
            <div class="px-4 pb-4 space-y-3" x-show="mode !== 'draft'" x-cloak>
                <div x-show="mode === 'jadwalkan'" x-cloak>
                    ...
                    <input type="datetime-local" name="mulai_tayang" value="{{ old('mulai_tayang') }}"
                ...
                <div>
                    ...
                    <input type="datetime-local" name="selesai_tayang" value="{{ old('selesai_tayang') }}"
