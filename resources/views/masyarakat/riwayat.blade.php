...
<div class="px-4 py-3.5 bg-white border-b border-stone-200 shrink-0">
    <p class="font-bold text-[1.0625rem] text-stone-900 tracking-tight">Riwayat Pengaduan</p>
...
<div x-data="{ filter: 'semua' }">
    ...
    {{-- Filter chips --}}
    <div class="px-3.5 py-3 flex gap-2 overflow-x-auto scrollbar-none shrink-0 bg-white border-b border-stone-100">
        @foreach(['semua' => 'Semua', 'menunggu' => 'Menunggu', 'proses' => 'Diproses', 'selesai' => 'Selesai', 'tidak_valid' => 'Tidak Valid'] as $val => $lbl)
    ...
    <div class="p-3.5 flex flex-col gap-2.5">
        ...
        <a href="{{ route('masyarakat.pengaduan.show', $p->id_pengaduan) }}"
           ...
           class="block bg-white rounded-xl p-4 shadow-sm no-underline border-l-[3px] {{ $statusCfg['border'] }} hover:shadow-md transition-shadow">

            <div class="flex items-start justify-between gap-2 mb-2.5">
                <p class="text-sm text-stone-800 leading-relaxed flex-1">{{ Str::limit($p->isi_laporan, 90) }}</p>
                <div class="flex flex-col items-end gap-1.5 shrink-0">
                    ...
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[0.6875rem] font-medium {{ $katCfg }}">
                        {{ ucfirst($p->kategori) }}
                    </span>
                </div>
            </div>
            ...
            <div class="mt-3 pt-3 border-t border-stone-100">
                <p class="text-[0.6875rem] font-semibold text-stone-400 mb-1">Tanggapan terakhir:</p>
                <p class="text-[0.8125rem] text-stone-700 leading-relaxed">{{ Str::limit($p->tanggapan->last()->tanggapan, 80) }}</p>
