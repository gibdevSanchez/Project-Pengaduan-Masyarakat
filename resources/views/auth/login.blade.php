...
<div x-data="{ guard: '{{ old('guard', 'masyarakat') }}', showPwd: false }">
    ...
    <h2 class="text-2xl font-bold text-stone-900 mb-1">Selamat datang</h2>
    <p class="text-sm font-light text-stone-500 mb-8">Masuk untuk melanjutkan ke M-Lapor</p>
    ...
    <div class="mb-7">
        ...
        <div class="relative flex p-1 bg-stone-100 rounded-2xl border border-stone-200">
            ...
            <div :class="guard === 'masyarakat'
                    ? 'translate-x-0 shadow-[0_2px_10px_rgba(232,112,74,0.28),0_1px_3px_rgba(0,0,0,0.07)]'
                    : 'translate-x-full shadow-[0_2px_10px_rgba(59,130,246,0.28),0_1px_3px_rgba(0,0,0,0.07)]'"
                 class="absolute top-1 left-1 bottom-1 w-[calc(50%-4px)] bg-white rounded-xl pointer-events-none transition-all duration-[280ms] ease-[cubic-bezier(0.4,0,0.2,1)]">
            </div>
    ...
    <form method="POST" action="/login">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <button type="button" @click="showPwd = !showPwd"
                            class="absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded text-stone-400 hover:text-stone-700 transition-colors">
        ...
        {{--
            bg-orange-500 = solid fallback before Alpine loads.
            :style switches the background-image (gradient) without conflicting with bg-orange-500
            since background-image and background-color are separate CSS properties.
        --}}
        <button type="submit"
                ...
                class="w-full py-3.5 px-4 rounded-xl bg-orange-500 text-white text-[0.9375rem] font-semibold border-0 cursor-pointer shadow-lg transition-all duration-150 hover:-translate-y-0.5 active:translate-y-px">
            Masuk
        </button>
    </form>
    ...
    <p x-show="guard === 'masyarakat'" x-cloak class="mt-6 text-center text-sm text-stone-400">
        Belum punya akun?
        <a href="{{ route('register') }}" class="font-semibold text-orange-500 hover:underline">Daftar di sini</a>
    </p>
</div>
@endsection
